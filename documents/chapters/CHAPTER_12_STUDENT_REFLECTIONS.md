...
In the end, I made a pragmatic decision: rather than continue trying to perfectly replicate the WordPiece algorithm, I implemented a simplified word-level tokeniser that splits text by whitespace and looks up whole words in a vocabulary file. This is not as accurate as full WordPiece tokenisation, particularly for less common words that need sub-word decomposition, and it means the actual on-device accuracy is lower than the 99.06% measured in the Python evaluation. I documented this honestly as a limitation (Section 10.4.8). In a production system, properly solving this — either by implementing a correct Kotlin WordPiece tokeniser or using TFLite's built-in tokenization support — would be a priority.
